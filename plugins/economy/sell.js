  ...
  async run({ sock, msg, sender, args }) {
    ...
    if (!args.length) {
      ...
      const lines = [...count.entries()].map(([name, quantity]) => {
        ...
        return `${getItemDefinition(name)?.emoji || "📦"} *${label(name)}* ×${quantity} → ${price ? `${formatRyu(price)} each` : "not sellable"}`;
      });
    ...
    if (!price) return reply(`❌ *${label(itemName)}* is not a recognized sellable item.`);
