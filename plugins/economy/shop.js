...
function buildCategoryList(categoryKey) {
  ...
  const itemLines = entries.map(([name, item]) => [
    `*${allKeys.indexOf(name) + 1}.* ${item.emoji}  *${displayName(name)}*`,
...
async function handleBuy(sock, msg, jid, sender, args) {
  ...
  const selected = Number.isInteger(itemNumber) && itemNumber >= 1 ? entries[itemNumber - 1] : null;
  if (!selected) return reply("❌ Invalid item number. Browse *.shop <category>* first, then use the number shown.");
  ...
  const missing = [];
  if (needCoins > userCoins) missing.push(`💰 Need ${formatRyu(needCoins - userCoins)} more`);
  if (needOrbs > userOrbs) missing.push(`🔮 Need ${needOrbs - userOrbs} more orbs`);
  if (needGems > userGems) missing.push(`💎 Need ${needGems - userGems} more diamonds`);
  if (missing.length) return reply(`💸 *Insufficient funds for ${displayName(itemName)}*\n\n${missing.join("\n")}`);

  if (item.bankTier) {
    const currentTier = Math.max(0, Math.floor(Number(user.bankUpgradeLevel) || 0));
    if (item.bankTier <= currentTier) return reply(`🏦 You already have this bank limit or a higher one (${formatRyu(bankLimitForUser(user))}).`);
  ...
  return reply([
    ...
    item.description ? `┃  📖 _${item.description}_` : null, "┃", DIV,
    `┃  💰 Paid  ꔫ ${costLine(item)}`,
    gunExpiry ? `┃  ⏳ Gun active for ${formatDuration(gunExpiry - Date.now())}` : null,
    DIV, `┃  💼 Wallet ꔫ ${formatRyu(user.money)}  ·  🔮 ${user.orbs}  ·  💎 ${user.diamonds}`,
    DIV, "┃  📦 *.inventory* to see your items", "╰━━━━━━━━━━━━━━━━━━━━━━━━━━╯",
  ].filter(Boolean).join("\n"));
}
