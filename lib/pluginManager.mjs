...
function discordGroupJid(guildId) {
  return `discord:${guildId}@g.us`;
}
...
async function discordGroupMetadata(guild) {
  const members = discordMembers(guild);
  return {
    id: discordGroupJid(guild.id),
    subject: guild.name,
...
function discordGroupSocket(client, message, fallbackChannel) {
  ...
  return {
    groupMetadata: async () => discordGroupMetadata(guild),
  ...
  return Object.assign(message, {
    ...
    key: {
      ...
      remoteJid: isDiscordGroup
        ? discordGroupJid(message.guild.id)
        : message.channelId,
