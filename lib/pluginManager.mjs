...
export async function routeDiscordMessage(client, message, prefix = ".", ownerId = "") {
  ...
  for (const plugin of plugins) {
    if (typeof plugin.onDiscordMessage !== "function") continue;
    try {
      await plugin.onDiscordMessage({ client, message, prefix });
    } catch (error) {
      log("warn", `Discord message hook ${plugin.name} failed: ${error.message}`);
    }
  }
