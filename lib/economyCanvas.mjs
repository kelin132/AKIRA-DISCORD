...
export async function generateMoneyThumbnail({ direction }) {
  const label = String(direction).toLowerCase() === "deposit" ? "DEPOSIT" : "WITHDRAW";
  const width = 320;
  const height = 320;
  const canvas = createCanvas(width, height);
  const ctx = canvas.getContext("2d");
  ...
  const background = ctx.createLinearGradient(0, 0, width, height);
  background.addColorStop(0, "#063b2a");
  background.addColorStop(1, "#0b1714");
  ctx.fillStyle = background;
  ctx.fillRect(0, 0, width, height);
  ...
  ctx.rotate(-0.08);
  ctx.fillStyle = "#83e38f";
  ctx.fillRect(-104, -62, 208, 124);
  ctx.strokeStyle = "#0b6b42";
  ctx.lineWidth = 5;
  ctx.strokeRect(-104, -62, 208, 124);
  ctx.fillStyle = "#0b6b42";
  ctx.beginPath();
  ctx.arc(0, 0, 34, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#c9f7c2";
  ctx.font = "bold 42px Arial";
  ctx.textAlign = "center";
  ctx.fillText("$", 0, 15);
  ctx.font = "bold 16px Arial";
  ctx.fillText("AKIRA BANK", 0, -78);
  ctx.restore();

  ctx.fillStyle = "#d7ffe0";
  ctx.font = "bold 24px Arial";
  ctx.textAlign = "center";
  ctx.fillText(label, width / 2, 268);
  ctx.font = "16px Arial";
  ctx.fillStyle = "#83e38f";
  ctx.fillText("MONEY TRANSFER", width / 2, 294);
