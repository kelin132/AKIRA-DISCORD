...
function customEmbed(content, options, fileName, thumbnailFileName) {
  ...
  if (spec.thumbnail) {
    const thumbnailUrl = spec.thumbnail === "thumbnail-attachment"
      ? `attachment://${thumbnailFileName}`
      : spec.thumbnail === "attachment"
        ? `attachment://${fileName}`
        : String(spec.thumbnail);
...
export function toDiscordPayload(content, options = {}) {
  ...
  if (content.discordEmbed) {
    ...
    const thumbnailFileName = content.thumbnailFileName || "thumbnail.png";
    const files = [];
    if (content.image !== undefined) files.push(toAttachment(content.image, fileName));
    if (content.discordThumbnail !== undefined) {
      files.push(toAttachment(content.discordThumbnail, thumbnailFileName));
    }
    if (files.length) payload.files = files;
    payload.embeds = [customEmbed(content, options, fileName, thumbnailFileName)];
    return payload;
  }
