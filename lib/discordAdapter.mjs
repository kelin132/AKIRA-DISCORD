  ...
  async sendMessage(chatId, content, options = {}) {
    ...
    if (content.text) {
      const embed = new EmbedBuilder()
        .setColor("#FF69B4") // Aidoru Pink
        .setDescription(content.text);
        
      if (content.linkPreview) {
        embed.setTitle(content.linkPreview.title || "Aidoru Link");
        if (content.linkPreview.jpegThumbnail) {
          const attachment = new AttachmentBuilder(content.linkPreview.jpegThumbnail, { name: "thumbnail.jpg" });
