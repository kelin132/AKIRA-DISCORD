...
function winnerMention(ticket) {
  const discordId = discordIdFrom(ticket.discordId || ticket.userId);
  return discordId ? `<@${discordId}>` : `**${lotteryDisplayName(ticket)}**`;
}
...
export async function drawLottery({
  ...
  const winners = weightedWinners(lot.tickets, 3);
  ...
  const winnerLines = awarded.map((winner, index) => {
    const medal = ["🥇", "🥈", "🥉"][index] || "🏆";
    return `┃ ${medal} ${lotteryDisplayName(winner)} ➜ ${winnerMention(winner)}`;
  });
  const mentions = awarded
    .map((winner) => discordIdFrom(winner.discordId || winner.userId))
    .filter(Boolean)
    .map((id) => `discord:${id}`);
