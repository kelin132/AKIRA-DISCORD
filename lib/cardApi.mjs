...
/** Build the Discord embed used by both automatic card spawns and summons. */
export async function buildDiscordCardSpawnPayload(card, spawnId, prefix = ".") {
  const embed = {
    title: "🃏 𝐂𝐀𝐑𝐃 𝐒𝐏𝐀𝐖𝐍",
...
/** Build the detailed summon embed shown after a user pays for a card. */
export async function buildDiscordSummonPayload(
  card,
  { summonCost = 0, walletRemaining = 0, prefix = "." } = {},
) {
  ...
  const extension = card.mediaType === "gif" || card.tierNum === "6" || card.tierNum === "S"
    ...
    : "jpg";
  const embed = {
    title: "🌟 Card Summoned Successfully",
    description:
      `A new card has been summoned and is ready to claim.\n\n` +
      `Use \`${prefix}claim ${card.cardId}\` to add it to your collection.`,
    color: 0xFF7A59,
    fields: [
      { name: "Card", value: String(card.name || "Unknown"), inline: false },
      { name: "Tier", value: `${tierEmoji} ${String(card.tier || "Common")}`, inline: false },
      { name: "Series", value: String(card.series || "Unknown"), inline: false },
      { name: "Card ID", value: `\`${card.cardId}\``, inline: false },
      { name: "Summon cost", value: `$${Number(summonCost).toLocaleString("en-US")}`, inline: false },
      {
        name: "Wallet remaining",
        value: `$${Number(walletRemaining).toLocaleString("en-US")}`,
        inline: false,
      },
    ],
    footer: { text: `Use ${prefix}claim ${card.cardId} to collect it.` },
  };
  const payload = { embeds: [embed] };

  if (card.media) {
    const url = await resolveMediaUrl(card.media);
    embed.image = { url: `attachment://summon.${extension}` };
    payload.files = [{ attachment: url, name: `summon.${extension}` }];
  }
...
export async function fetchCardByTier(tier) {
  ...
  try {
    const cached = TIER_CACHE.get(num);
    const raw = cached && Date.now() - cached.fetchedAt < TIER_CACHE_TTL_MS
      ? cached.cards
      : await fetchTier(num);
    if (!cached || Date.now() - cached.fetchedAt >= TIER_CACHE_TTL_MS) {
      TIER_CACHE.set(num, { cards: raw, fetchedAt: Date.now() });
    }
