...
**How to apply:** Preserve normal npm install scripts for deployment, keep the builder/runtime system packages aligned whenever media plugins are changed, and run the bootstrap before importing application modules when hosts may invoke `node index.js` directly.